...
const SELENIUM_HOST = process.env.SELENIUM_HOST ?? "SELENIUM_HOST";
const OPTIONS = [
  "headless", //ヘッドレスの方が早い
  "no-sandbox", //なんだっけ
...
router.get("/", async (req, res) => {
  console.log("buiiding...");
  const driver = await new Builder()
    .forBrowser("chrome")
    .usingServer(SELENIUM_HOST)
    .setChromeOptions(new Options().addArguments(...OPTIONS))
    .build();
  ...
  try {
    ...
    const tr_texts = (
      await Promise.all(
        el_server_trs.map(async (el: any) => {
